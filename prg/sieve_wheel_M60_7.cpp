// sieve_wheel_M60_7.cpp : Questo file contiene la funzione 'main', in cui inizia e termina l'esecuzione del programma.
...
#include <string>   // Per std::stoull (lettura da riga di comando)
...
// NON PIU' CONST: Variabili globali modificabili da riga di comando
u64 CERCA_IN = 1000000000ULL;
u64 N_SOTTOLISTE = 0;
...
// Lettura degli argomenti da terminale (argc, argv)
int main(int argc, char* argv[]) {
    // Se c'è un numero scritto dopo il nome del programma, lo usiamo
    if (argc > 1) {
        try {
            CERCA_IN = std::stoull(argv[1]);
        }
        catch (...) {
            std::cerr << "Errore: Inserisci un numero valido come parametro." << std::endl;
            return 1;
        }
    }

    // Calcoliamo la dimensione effettiva
    N_SOTTOLISTE = (CERCA_IN / MODULO) + 1;
    ...
    std::cout << "--- Sieve Wheel M60 ricerca su: " << CERCA_IN << " (Pre-filtro 7 Memcpy) ---" << std::endl;

    uint16_t maschere_init[7] = { 0 };
    for (int i = 0; i < 7; i++) {
        for (int b = 0; b < 16; b++) {
            if (SOTTOLISTE[i][b] != 0) {
                maschere_init[i] |= (1 << b);
    ...
    try {
        lista_completa.resize(N_SOTTOLISTE);
        uint16_t* ptr = lista_completa.data();

        // --- INIZIALIZZAZIONE ULTRA-VELOCE (MEMCPY DOUBLING) ---
        if (N_SOTTOLISTE <= 7) {
            for (u64 i = 0; i < N_SOTTOLISTE; i++) ptr[i] = maschere_init[i];
        }
        else {
            for (int i = 0; i < 7; i++) ptr[i] = maschere_init[i];
            u64 filled = 7;
            while (filled * 2 <= N_SOTTOLISTE) {
                std::memcpy(ptr + filled, ptr, filled * sizeof(uint16_t));
                filled *= 2;
            }
            if (N_SOTTOLISTE > filled) {
                std::memcpy(ptr + filled, ptr, (N_SOTTOLISTE - filled) * sizeof(uint16_t));
            }
        }
        // --------------------------------------------------------
    }
    catch (...) {
        std::cerr << "Errore: RAM insufficiente per allocare " << CERCA_IN << std::endl;
        return 1;
    }
    ...
    for (u64 p = 11; p <= limite; p++) {
        if (!is_prime_simple(p)) continue;
        PrimoAtleta pa;
        pa.p = p;
        u64 n = p * p;
        int p_num = (int)((n - 10) % 60);
        u64 p_lista = (n - 10) / 60;
        int delta = (2 * p) % 60;
    ...
    for (u64 i = 1000; i < 1100; i++) {
        ...
        for (int b = 0; b < 16; b++) {
            if (mask & (1 << b)) {
                ...
                for (u64 p : {11, 13, 17, 19, 23}) if (n > p && n % p == 0) fallimenti++;
            }
        }
    }
